 ...
 * @returns - result
 */
function spliceUrlParam(params: Record<string, unknown>, covert = false, withQuestionsMark = true) {
	if (!params) {
	...
	const result: string[] = []
	for (key in params) {
		result.push(`${key}=${encodeURIComponent('' + (covert ? params[key] ?? '' : params[key]))}`)
	}
