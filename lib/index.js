...
function getAppVersion(appName, withappstr, userAgent) {
  ...
  var ver = userAgent.match(reg); // withappstr = typeof(withappstr) != "undefined" ? withappstr : false;
...
function handleEvent(event) {
  var returnValue = true; // @ts-ignore

  var that = this; //抓获事件对象(IE使用全局事件对象)

  event = event || fixEvent(((that.ownerDocument || that.document || that).parentWindow || window).event); // 取得事件处理函数的哈希表的引用
  // @ts-ignore
...
function downloadUrlFile(url, filename) {
  var xhr = window.XMLHttpRequest ? new XMLHttpRequest() : // @ts-ignore
