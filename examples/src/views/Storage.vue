...
const handleGetCache = () => {
	lsResult.value = getCache(lsKey.value)
}
const handleDelCache = () => {
	delCache(lsKey.value)
	lsResult.value = null
}
...
const handleGetSession = () => {
	ssResult.value = getSession(ssKey.value)
}
const handleDelSession = () => {
	delSession(ssKey.value)
	ssResult.value = null
}
...
const handleSetCookie = () =>
	setCookie(cookieKey.value, cookieValue.value, cookieDays.value * 24 * 60 * 60)
const handleGetCookie = () => {
	cookieResult.value = getCookie(cookieKey.value)
}
const handleGetAllCookies = () => {
	allCookies.value = getCookies()
}
const handleDelCookie = () => {
	delCookie(cookieKey.value)
			...
			<template #input>
				<n-space vertical>
					...
					<n-space>
						...
						<n-button size="small" type="error" @click="handleDelCache"
							>Delete</n-button
						>
					</n-space>
				</n-space>
			...
			<template #input>
				<n-space vertical>
					...
					<n-space>
						...
						<n-button size="small" type="error" @click="handleDelSession"
							>Delete</n-button
						>
					</n-space>
				</n-space>
			...
			<template #input>
				<n-space vertical>
					...
					<n-space>
						...
						<n-button size="small" type="error" @click="handleDelCookie"
							>Delete</n-button
						>
