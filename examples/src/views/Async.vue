...
console.log('Data:', data)`"
    >
      <template #input>
        <n-space align="center">
          <n-button size="small" @click="fetchData(false)" :disabled="awaitToLoading">Fetch Success</n-button>
          <n-button size="small" type="error" @click="fetchData(true)" :disabled="awaitToLoading">Fetch Failure</n-button>
          <span v-if="awaitToLoading" style="font-size: 12px; color: #666;">Loading...</span>
        </n-space>
      </template>
      <template #result>
...
retry(fn, { onRetry: (err, attempt) => console.log(attempt) })`"
    >
      ...
      <template #result>
        <n-tag v-if="retryResult" type="info" size="small" :bordered="false">{{ retryResult }}</n-tag>
      </template>
    </FunctionCard>
  </div>
</template>
