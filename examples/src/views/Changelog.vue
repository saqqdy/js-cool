  ...
  <div>
    ...
    <div v-for="group in groupedChanges" :key="group.type" style="margin-bottom: 32px;">
      <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 16px;">
        <n-icon size="20" :component="typeConfig[group.type].icon" :color="typeConfig[group.type].color === 'success' ? '#18a058' : typeConfig[group.type].color === 'info' ? '#2080f0' : typeConfig[group.type].color === 'warning' ? '#f0a020' : '#d03050'" />
        <h3 style="margin: 0; font-size: 16px; font-weight: 600;">
          {{ locale === 'zh' ? typeConfig[group.type].labelZh : typeConfig[group.type].label }}
        </h3>
        <n-tag size="small" :type="typeConfig[group.type].color">{{ group.items.length }}</n-tag>
      </div>
          ...
          >
            <div class="changelog-item">
              <div style="flex: 1; min-width: 0;">
                ...
                <p class="text-muted" style="font-size: 13px; margin: 0;">{{ getDescription(item) }}</p>
              </div>
              <n-icon class="text-muted" style="flex-shrink: 0;"><ArrowForwardOutline /></n-icon>
            </div>
          </router-link>
    ...
    <div class="legend-box">
      <p style="font-size: 12px; margin: 0;">
        {{ locale === 'zh' ? '提示：点击条目可跳转到对应方法的详细用法' : 'Tip: Click on an item to navigate to the function for detailed usage' }}
      </p>
    </div>
  </div>
</template>
